...
- **详见**：[`plans/taxonomy-refactor.md`](./plans/taxonomy-refactor.md)。
...
- 可能需多轮才能覆盖全部 727。
- **详见**：[`plans/diabolical-727.md`](./plans/diabolical-727.md)（骨架；详细规划待 ① 完成后重新分析）。
...
1. **快速 smoke / soundness（贯穿始终）**——`data/ground-truth/`（各档 100，共 400 题，带解）。`npm test` 的 AC-3 跑全 400 题断言 0 violation。便宜、每次改动都跑。这是**抽样**，能过不代表全语料无问题。
2. **重型回归门槛**——完整语料 `npm run corpus:run`。**任何引擎改动**（① 重构、② 加策略）都跑：要求 solved 数不回退、0 invalid。抽样漏掉的 soundness 问题靠它兜底。
3. **Roadmap ② 目标/进度集**——`data/failing-diabolical/`（727 题，无需解）。**不**进 `npm test` 硬门槛（起点 0/727）。进度用 `corpus:run --difficulty diabolical` 跟踪；每攻克一题在 `diabolical-regressions.test.ts` 加一条针对性回归。

## 运行与验证

```bash
npm install
npm test            # 第 1 层:单元 + 语料集成 + AC-3 soundness（含 diabolical 回归）
npm run typecheck
npm run gen:ground-truth   # 重新生成冻结标准答案集
npm run corpus:run -- --difficulty diabolical --limit 50   # 第 2 层冒烟:对当前引擎跑全语料子集
```
