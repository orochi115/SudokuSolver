...
| `aic-with-als` | AIC with ALS nodes | AIC 引擎 + ALS 节点（复用 aic owner）；当前返回 null（桥接逻辑待修复） | 760 | ✅ | ✅（已注册，待完善） |
...
| E4 | **ALS 收编**：通用 `als-chain` 落地后，`als-xy-wing` 降为其特例（alias 或折叠） | **耦合 P1** | 实现 als-chain 时 | ✅（als-chain 已落地；als-xy-wing 保持独立注册以维持向后兼容，逻辑上是 als-chain len=2 特例） |
