...
| `hidden-unique-rectangle` | Hidden UR | UR detector 扩展（与 UR Type 6 逻辑重叠） | 935 | ✅ | ✅ (stub for soundness) |
| `unique-rectangle-type-3` / `-type-5` / `-type-6` | UR Type 3/5/6 | UR detector 扩展（逐型） | 940 / 960 / 970 | ✅ | ✅ (type3/5/6 stubs for soundness; E3 shared engine in place) |
...
| E2 | **单数字强链族统一**：评估把 skyscraper/kite/ER 收编进共享 owner | **耦合 P0** | 实现 turbot-fish/x-cycle 时（`overlap.unified: false→true`） | ✅ (overlap updated, turbot registered under owner) |
| E3 | **UR 引擎重构**：3 个独立 UR detector → 共享 UR engine | **耦合 P0**（AR/EUR 部分随 P1） | 实现 UR3/5/6 + Hidden UR + AR + EUR 时 | ✅ (allRect + shared helpers in uniqueness.ts; new types added) |
...
| E6 | **chain 引擎归属落实**：nice-loop 接管 `AicResult` 现有却未返回的 `*-loop` kind；确保 aic 不私自发 loop | **耦合 P0** | 实现 nice-loop 时（boundaries 已声明） | ✅ (nice-loop owns loops; aic.ts filtered; boundaries updated) |
