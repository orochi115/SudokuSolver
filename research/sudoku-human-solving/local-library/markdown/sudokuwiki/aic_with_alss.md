Title: AIC with ALSs - SudokuWiki.org

URL Source: https://www.sudokuwiki.org/AIC_with_ALSs
...
[Print Version](https://www.sudokuwiki.org/Print_AIC_with_ALSs)
...
AIC with ALS : [Load Example](https://www.sudokuwiki.org/sudoku.htm?bd=S9B06030918071810080143430710060309160s020504080901060307b6cy052i0l060p7u0o041h03090l08071f059f381f03052i088r1o2r040806032q0z02099y9e0201089u043m1i9zab1f2s049w132r08) or : [From the Start](https://www.sudokuwiki.org/sudoku.htm?bd=600070000007003900250090030005006000403000705000300800040030029002100400000040008)
 ...
 A real life example now. This chain contains an ALS on the cells {G6,H6} (I used squiggly brackets to denote ALS as opposed to square brackets for [Grouped Cells](https://www.sudokuwiki.org/Grouped_X_Cycles)). 9s in row H are the entry point. We turn 9 ON in H2 which turns OFF the 9 in H6 - the extra candidate that makes the ALS an ALS. This gives us a Naked Pair of [5,7] that points up column 6 turning OFF the 7 in F6 and the chain continues. 

Ultimately we use [Nice Loop Rule 2](https://www.sudokuwiki.org/Alternating_Inference_Chains) to place 4 in A4
...
Missing chain revealed : [Load Example](https://www.sudokuwiki.org/sudoku.htm?bd=S9Bckbm0a22030g8s8s8k8m7u1q020a0h8u050707181s22828i08011k0164090c0f050s2k444k06107n0g047p03bp122y1a7n08028rai8z90850807848m0n8k0404851w1u848m0g080n8k03070h0d01058k8k) or : [From the Start](https://www.sudokuwiki.org/sudoku.htm?bd=000030000000200057700000810109005000060004030000082000008700004400000080037001500)
 ...
 I've noticed some missing eliminations having tried some examples from Hodokus exellent page on [Chains and Loops](https://hodoku.sourceforge.net/en/tech_chains.php). Here is one. Coming out of an ALS in a loop the solver wasn't detecting that the exit landed on the starting candidate. Now does.
...
Extra off-chain eliminations : [Load Example](https://www.sudokuwiki.org/sudoku.htm?bd=S9B5u0e0204030a090f5u4a030f077o7o4a010501092i4y5e5e0u022m022b5z7rdmdq4n4m0609045z53767a4p4m490e0f4749444807040i032d095070704t6a4d062c0ebc01dc4g5y4g2i082j052c2g06090p) or : [From the Start](https://www.sudokuwiki.org/sudoku.htm?bd=052431960036700015190000020200000006940000000560000749309000000605010000080500690)
 ...
 A more important observation is that an ALS produces a Naked Pair and both candidates in the pair can contribute to off-chain eliminations. Provided the loop connects and we can be sure of the normal eliminations we should also look at the whole Naked Pair.

In Hodoku's example the chain is coming in from +3 in J6 to knock off the 3 in F6. Using the 2s in F56 which are now necessary for those cells, the chain continues to -2 F4 and onwards before finishing on J9 where it started.
...
Go back to [AICs with Grouped Cells](https://www.sudokuwiki.org/AIC_with_Groups)Continue to [Using Unique Rectangles as Links in Chains](https://www.sudokuwiki.org/Using_Unique_Rectangles_as_Links_in_Chains)

* * *

[](https://www.sudokuwiki.org/AIC_with_ALSs)
...
[](https://www.sudokuwiki.org/AIC_with_ALSs)Comments[Talk](https://www.sudokuwiki.org/AIC_with_ALSs?talk#comments)
...
REPLY TO THIS POST
 ...
 Article created on 12-April-2008. Views: 82439

 This page was last modified on 1-December-2025.
