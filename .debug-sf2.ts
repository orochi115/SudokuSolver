...
for (const baseCombo of combineIndices(eligible.length, size)) {
  ...
  if (!baseIndices.includes(0)) continue;
  ...
  for (const coverCombo of combineIndices(9, size)) {
    ...
    for (let fb = 0; fb < 9; fb++) {
      ...
      if (!finCells.includes(FIN1) || !finCells.includes(FIN2)) continue;
      ...
      if (elims.includes(TARGET)) {
        console.log(`CANONICAL base${baseIndices.map(i=>i+1)} cover${[...coverSet].map(i=>i+1)} finBox${fb+1} fins${finCells.map(label)} elims${elims.map(label)}`);
      }
    }
  }
}