...
describe('gate 6 — chain-engine boundaries', () => {
  ...
  it('P3 chain and chain-adjacent strategies are multi-branch and last-resort only', () => {
    for (const id of P3_CHAIN_IDS) {
      expect(MULTI_BRANCH_IDS.has(id), id).toBe(true);
      const forcing = CHAIN_OWNERSHIP.find((c) => c.strategyId === id)!;
      expect(forcing.profiles, id).toEqual(['last-resort']);
    }
  });
