...
function tryAvoidableRectangle(grid: Grid, type: 1 | 2 | 3 | 4): Step | null {
  // AR requires knowing which cells are givens vs solved cells.
  // We access the grid's given state through the isGiven method if available.
  // Grid interface: isGiven(cell): boolean
  // If not available, we skip AR (it would be unsound to guess).
  if (!('isGiven' in grid) || typeof (grid as unknown as { isGiven: (c: number) => boolean }).isGiven !== 'function') {
    return null;
  }
  const isGiven = (c: number): boolean =>
    (grid as unknown as { isGiven: (c: number) => boolean }).isGiven(c);
  ...
  for (const [c11, c12, c21, c22] of allRectangles2x2()) {
    ...
    for (const hIdx of sharedHouses) {
      ...
      for (const [locked, elim] of [[a, b], [b, a]] as [number, number][]) {
        ...
        // Check if locked is confined to open cells in this house
        const lockedInHouse = house.filter(
          (c) => grid.get(c) === 0 && (grid.candidatesOf(c) & lockedBit) !== 0,
        );
        const nonOpenWithLocked = lockedInHouse.filter((c) => !openCells.includes(c));
        if (nonOpenWithLocked.length !== 0) continue;

        // Locked confined to open cells → elim can be removed from open cells
