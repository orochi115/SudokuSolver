...
function retitleRemotePairs(step: Step): Step {
  return {
    ...step,
    strategyId: 'remote-pairs',
    explanation: {
      zh: `远程数对：${step.explanation.zh}`,
      en: `Remote Pairs: ${step.explanation.en}`,
    },
  };
}
...
export const remotePairs: Strategy = {
  ...
  apply(grid: Grid): Step | null {
    const step = tryRemotePairs(grid);
    if (step) return step;
    const xy = xyChain.apply(grid);
    return xy ? retitleRemotePairs(xy) : null;
  },
};
