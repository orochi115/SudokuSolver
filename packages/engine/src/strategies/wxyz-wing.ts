...
export const wxyzWing: Strategy = {
  ...
  apply(grid: Grid): Step | null {
    ...
    const checkPool = (C_pool: number[]): Step | null => {
      // Choose 4 cells from C_pool
      const n = C_pool.length;
      for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
          for (let k = j + 1; k < n; k++) {
            for (let l = k + 1; l < n; l++) {
              const combo = [C_pool[i]!, C_pool[j]!, C_pool[k]!, C_pool[l]!].sort((a, b) => a - b) as [number, number, number, number];
              ...
              if (popcount(union_mask) !== 4) continue;
              ...
              if (non_restricted_count === 1) {
                ...
                if (elims.length > 0) {
                  return {
                    ...
                    explanation: {
                      zh: `WXYZ翼：四个格 {${combo.map((c) => cellLabel(c)).join(', ')}} 包含四个数字 {${digits.join(',')}}；其中只有数字 ${z} 为非受限公共候选数，消去所有能同时看到这四个格中所有 ${z} 的格子的候选数 ${z}。`,
                      en: `WXYZ-Wing: four cells {${combo.map((c) => cellLabel(c)).join(', ')}} contain four digits {${digits.join(',')}}; only ${z} is non-restricted, so eliminate ${z} from cells seeing all ${z} in the pattern.`,
                    },
                  };
                }
              }
