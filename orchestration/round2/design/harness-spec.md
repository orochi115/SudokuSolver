...
1. **turn 制**：harness 驱动每策略实现，模型在同 round 单会话内多 action 迭代
2. **可恢复**：git checkpoint + append-only 事件；失败 turn 可 reset 重跑
...
flowchart TB
  ...
  pipe --> turn
  turn --> oc
  turn --> vt[verify-turn.sh]
  pipe --> vr[verify-round.sh]

  monitor -->|read| status[status + jsonl]
  monitor -->|touch| ctl
  ctl -->|pause/stop| pipe
  wd -->|kill + cost fuse| pipe
  turn -->|append| events[events.jsonl]
```
...
| `run-round.sh` | 单模型单 round：遍历 turns → round 末 verify |
| `run-turn.sh` | 单 turn：action 循环 → verify-turn → git commit → checkpoint |
| `verify-turn.sh` | 夹具 + typecheck 增量 + 单文件 oracle 扫描 |
| `verify-round.sh` | npm test + verify:r2 全量（727 + 污染 + P3） |
| `watchdog.sh` | stale 检测 → kill + 留痕；费用累加 → fuse |
| `monitor.sh` | 只读仪表盘；`stop`/`pause` 转 control 文件 |

**费用**：检测/熔断在 **watchdog**（或 run-turn 每 action 后累加）；展示在 **monitor**。
...
- 模型在 turn 内可调整**本 id** 的 `difficulty` 与在 `CANONICAL_STRATEGY_ORDER` 中的位置
- harness lint（verify-turn）：禁止单 turn 内改动其他 id 的 difficulty；禁止单 turn 改动 `LAST_RESORT_IDS`（非 p3 round）
...
| 迁移 | 条件 |
|---|---|
| 进入下一 round | 当前 round `round.complete` |
| `round.skip` | 当前 round `round.fail` 且策略配置为 STOP 链 |
| `round.fail` 但继续 | 配置 `ROUND_FAIL_POLICY=continue` 时仅记分，不 SKIP（实验选项） |
...
```
for strategyId in required-ids/<round>.txt (ordered):
  turn.start
  loop actions with ACTION_RETRIES
  verify-turn
  on success: checkpoint, turn.complete
  on fail: TURN_RETRIES with git reset to parentCommit
  on exhaust: turn.fail → ROUND_FAIL or turn.skip per policy
```

### 4.3 Action 循环（turn 内）

```
action.start
opencode run (continue session within turn)
action.end
if watchdog/idle/exit!=0 → action retry (same turn, same or new session per config)
if pause file → pause.accepted, exit runner gracefully
else continue until model turn-done signal or idle timeout
```

**Action 失败不直接 turn.fail**；耗尽 `ACTION_RETRIES` 才进入 verify-turn 或下一 action 策略。

## 5. 验证分层

| 层级 | 命令/脚本 | 时机 | 硬门 |
|---|---|---|---|
| T0 | 策略夹具单测 `vitest run <id>.test.ts` | turn 末 | ✅ |
| T1 | `apply(fixtureGrid) !== null` smoke | turn 末 | ✅ |
| T2 | typecheck | turn 末 | ✅ |
| T3 | 单文件 brute-oracle grep | turn 末 | ✅ |
| R1 | `npm test`（含 400 GT soundness） | round 末 | ✅ |
| R2 | `verify:r2 --phase <round>`（727 + 污染 + P3） | round 末 | ✅ |
| KPI | 策略利用率（见 scoring.md） | round 末 | ❌ 评分 |

**ground-truth（400）**：不在每个 turn 全跑；在 **round 末** 随 `npm test` 执行。turn 级用**夹具**触发策略，避免「400 未触发策略却通过」。
...
|---|---|---|
| action | （无；仅 retry） | — |
| turn | 夹具失败；smoke null；typecheck；本文件 oracle；越权改 profiles | — |
| round | npm test；727 soundness；污染；P3 泄漏；本 round 全部 turn 未完成 | 利用率、727 delta、缺实现 id 列表 |
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `ACTION_RETRIES` | 3 | 单次 opencode 失败、watchdog kill、idle |
| `TURN_RETRIES` | 2 | 含 reset 到新 session + parentCommit |
| `ROUND_RETRIES` | 1 | 仅 round 末 verify 失败；重新跑末段 turns 或全 round |

## 8. 摆烂检测（id 预写死）

| 检测 | 说明 |
|---|---|
| 夹具未通过 | turn 无法 complete |
| `apply` 在夹具盘面返回 null | turn 无法 complete |
| round 末扫描 | 本 round required id 均存在 `turn.complete` 事件 |
...
## 11. Prompt 结构（每 action）

harness 拼接（非模型自选）：

1. round/turn 上下文（当前 id、已完成 id 列表）
2. 本 id 研究卡路径 + 夹具要求
3. 红线（oracle、profiles）
4. 若 retry：结构化失败（`verify-turn` / `action.end` 摘要）
5. **禁止**一次实现多个 id
...
```bash
git commit -m "model/<name>: <round>/<strategyId> ok (action=N)"
...
# 新 action 序列；prompt 含「上轮失败摘要」
```

report / 人工可据 `checkpoints.jsonl` 检出任意 turn 起点复现。
...
- [ ] pause 不产生 `round.fail`
- [ ] resume 读 jsonl 不读孤立 metrics
- [ ] turn 失败可 `git reset` 到 `parentCommit`
- [ ] report.sh 仅聚合 jsonl
